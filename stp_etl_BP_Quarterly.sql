...
CREATE PROCEDURE [dbo].[stp_etl_BP_Quarterly]
...
DECLARE @MAX_DATE AS DATE = (SELECT ISNULL(MAX(DATE), '1900-01-01') FROM [bp].[Quarterly])
	...
	SELECT Q.January
		...
		LEFT JOIN [stg].[GBRCA] AS CA

		ON Q.March = CA.Date

		LEFT JOIN [stg].[GBRGCP] AS RGDP

		ON Q.March = RGDP.Date

		LEFT JOIN [stg].[GBRGD] AS DEFL
