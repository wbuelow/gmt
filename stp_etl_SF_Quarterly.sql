...
CREATE PROCEDURE [dbo].[stp_etl_SF_Quarterly]
...
DECLARE @MAX_DATE AS DATE = (SELECT ISNULL(MAX(DATE), '1900-01-01') FROM [sf].[Quarterly])
	...
	SELECT Q.January
		...
		LEFT JOIN [stg].[CHECA] AS CA

		ON Q.March = CA.Date

		LEFT JOIN [stg].[CHEGCP] AS RGDP

		ON Q.March = RGDP.Date

		LEFT JOIN [stg].[CHEGD] AS DEFL
